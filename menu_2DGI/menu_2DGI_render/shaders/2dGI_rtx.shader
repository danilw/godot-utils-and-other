...
vec4 add_text_col_glow(vec2 p){
    ...
    return vec4(col,max(clamp(smoothstep(0.015,0.,dist),0.,1.),clamp(a,0.,1.)));
}

void mainImage( out vec4 fragColor, in vec2 fragCoord, vec2 iResolution )
{
	vec2 ouv = fragCoord.xy / iResolution.xy;
	vec3 prevCol = texture(iChannel0, ouv).rgb;
    int SAMPLES=4;
	float TIMESPAN=40.0;
    ...
    col = prevCol*(1.0-st)+col*st*(1.-tc.a);
    col=tc.rgb*st+col;
	ouv.y=1.-ouv.y;
	col+=texture(iChannel1,ouv).rgb*st;
...
void fragment(){
	...
	if(iFrame==0)COLOR=vec4(vec3(0.),1.);
}