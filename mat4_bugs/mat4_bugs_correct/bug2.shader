...
void fake_vertex(inout mat4 mtx, inout mat4 tmtx) {
	//mtx=mat4(vec4(0.),vec4(0.),vec4(0.),vec4(0.,0.,1.,0.));//uncomment and comment next line, not same result
	//bug
	mtx=lookAt(vec3(0.,0.,1.),wmtx[3].xyz,vec3(0.,1.,0.));
	tmtx=mtx;
	
	vec3 a=vec3(1.,0.,0.);
	vec3 b=vec3(0.,1.,0.);
	
	translate(tmtx,a); //still do nothing
	
	//tmtx[3].xyz=a; //fix still work
...
void fragment() {
	...
	vec4 col=vec4(0.);
	if(UV.y<0.5)ALBEDO=vec3(abs(WORLD_MATRIX[3].y),0.,0.);
	else{
	if(UV.x<0.5)ALBEDO = mtx[3].xyz;
	else ALBEDO = tmtx[3].xyz; //this blue
	}
}
